(() => {
  ...
  function nextPrescription(name, actualSets, targetSets) {
    ...
    if (rule.mode === 'fixed') return targetSets.map(set => ({weight:set.weight ? roundTo(+set.weight + rule.increment, rule.increment) : set.weight,reps:set.reps}));
    const reachedTop = targetSets.every(set => (+set.reps || 0) >= rule.max);
    if (reachedTop) return targetSets.map(set => ({weight:set.weight ? roundTo(+set.weight + rule.increment, rule.increment) : set.weight,reps:rule.min}));
