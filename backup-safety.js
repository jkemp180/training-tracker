(() => {
  ...
  function validate(payload){
    if(!validObject(payload)||payload.format!=='hybrid-training-backup'||!validObject(payload.data))throw new Error('Not a Hybrid Training backup.');
    const d=payload.data;
    if(typeof d.startDate!=='string'||!/^\d{4}-\d{2}-\d{2}$/.test(d.startDate))throw new Error('Backup start date is invalid.');
    if(typeof d.weight!=='number'||!Number.isFinite(d.weight))throw new Error('Backup body weight is invalid.');
    if(typeof d.theme!=='string')throw new Error('Backup theme is invalid.');
    ['logs','prescriptions','history'].forEach(k=>{if(!validObject(d[k]))throw new Error(`Backup ${k} is invalid.`)});
    if(d.runLevels!==undefined&&!validObject(d.runLevels))throw new Error('Backup running progression is invalid.');
    return d;
  }
  function downloadBackup(){
    const payload=backupPayload();
    const url=URL.createObjectURL(new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}));
    const a=document.createElement('a');a.href=url;a.download=`hybrid-training-backup-${payload.createdAt.slice(0,10)}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);
    localStorage.setItem(META_KEY,JSON.stringify({lastBackupAt:payload.createdAt,completedAtBackup:completed()}));
    setTimeout(()=>render('settings'),100);
  }
  function backupCard(){const m=meta(),since=Math.max(0,completed()-(m.completedAtBackup||0));return `<section class="card stack backup-card"><div><p class="eyebrow">YOUR TRAINING DATA</p><h3>Backup</h3><p class="muted">Your Home Screen app stores its training history on this iPhone. Save a backup to iCloud Drive or another safe location.</p></div><div class="grid-2"><div class="metric"><span class="muted">Last backup</span><strong>${m.lastBackupAt?new Date(m.lastBackupAt).toLocaleDateString():'Never'}</strong></div><div class="metric"><span class="muted">New workouts</span><strong>${since}</strong></div></div>${since>=3||!m.lastBackupAt?`<div class="notice">${!m.lastBackupAt?'Create your first backup now.':`You have ${since} completed workouts since your last backup.`}</div>`:''}<button class="primary" id="safeExportBtn">Back up my data</button><button class="secondary" id="safeImportBtn">Restore from backup</button><p class="muted compact-help">Backup includes workout logs, actual sets, strength progression, running levels, weight, start date and settings.</p></section>`}

  const oldSettings=settingsView;
  settingsView=()=>backupCard()+oldSettings().replace('<button class="secondary" id="exportBtn">Export backup</button>','').replace('<button class="secondary" id="importBtn">Import backup</button>','');
  const oldBind=bind;
  bind=function(view){oldBind(view);if(view==='settings'){safeExportBtn.onclick=downloadBackup;safeImportBtn.onclick=()=>importInput.click()}};
